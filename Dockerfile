...
RUN sed -i 's/enabled=0/enabled=1/' /etc/yum.repos.d/CentOS-PowerTools.repo || true && \
    ...
    pip3 install --extra-index-url https://artifactory.vgt.vito.be/api/pypi/python-packages/simple https://artifactory.vgt.vito.be/python-packages-public-snapshot/worldcereal/0.1.2a1/worldcereal-0.1.2a1.20210521.47_develop-py3-none-any.whl && \
    rm -rf /root/.cache && \
    yum clean all && \
    rm -rf /var/cache/yum/*
